...
# ============================================================================
# Point d'entrée ajouté par Elie Bussod pour régénérer facilement des formules
# Tseytin via ce script.
#
# La classe TseytinTransformation ci-dessus est de Loïc Bardi, copiée sans
# modification depuis son projet de bachelor (UNIGE). Seul le bloc ci-dessous
# (__main__) est un ajout personnel pour exposer un CLI pratique.
#
# IMPORTANT : les fichiers `data/tseytin/{3,4,5,7,10}x{..}.dimacs` livrés avec
# le dépôt sont les fichiers d'origine de Loïc Bardi ; ils contiennent en fait
# 2k clauses unitaires qui fixent les bits de sortie z à une valeur spécifique
# (donc ce sont des instances de factorisation, pas des circuits purs).
# Ce script n'écrase JAMAIS ces fichiers :
#   - les circuits purs (sans contrainte z) sont écrits dans
#     `data/tseytin/pure_<k>x<k>.dimacs`
#   - les instances de factorisation en `data/factorization/<k>x<k>_z<z>.dimacs`
# ============================================================================
...
if __name__ == "__main__":
    ...
    tseytin_dir = os.path.abspath(os.path.join(script_dir, "..", "tseytin"))
    factor_dir  = os.path.abspath(os.path.join(script_dir, "..", "factorization"))

    # Tailles de circuits à régénérer.
    CIRCUIT_SIZES = [3, 4, 5, 7, 10]

    def regen_pure_circuits():
        os.makedirs(tseytin_dir, exist_ok=True)
        print("=== REGENERATION DES CIRCUITS TSEYTIN PURS ===")
        print(f"Sortie : {tseytin_dir}/pure_*.dimacs\n")
        for k in CIRCUIT_SIZES:
            t = TseytinTransformation()
            t.MULT(k, k)
            out = os.path.join(tseytin_dir, f"pure_{k}x{k}.dimacs")
            # parse_formula écrit déjà l'en-tête `p cnf n m`.
            TseytinTransformation.parse_formula(t.sat, out)
            with open(out) as f:
                header = f.readline().strip()
            print(f"  [OK] pure_{k}x{k}.dimacs ({header})")

    def regen_factorization(k, z):
        os.makedirs(factor_dir, exist_ok=True)
        # binaryConverter est dans le même dossier que ce script.
        from binaryConverter import dec_to_bin

        print(f"=== GENERATION FACTORISATION {k}x{k} avec z={z} ===")
        print(f"Sortie : {factor_dir}\n")
        t = TseytinTransformation()
        t.MULT(k, k)
        out = os.path.join(factor_dir, f"{k}x{k}_z{z}.dimacs")
        TseytinTransformation.parse_formula(t.sat, out)
        z_bin = dec_to_bin(z)
        TseytinTransformation.add_z_to_dimacs(z_bin, k, k, out)
        with open(out) as f:
            header = f.readline().strip()
        print(f"  [OK] {k}x{k}_z{z}.dimacs ({header})")

    # Modes d'utilisation :
    #   python3 tseytin.py                  # circuits purs -> tseytin/pure_*
    #   python3 tseytin.py factor <k> <z>   # factorisation k×k -> factorization/
    args = sys.argv[1:]
    if len(args) == 0:
        regen_pure_circuits()
    elif len(args) == 3 and args[0] == "factor":
        regen_factorization(int(args[1]), int(args[2]))
    else:
        print("Usage :")
        print("  python3 tseytin.py                  # circuits purs")
        print("  python3 tseytin.py factor <k> <z>   # factorisation k x k")
        sys.exit(1)
