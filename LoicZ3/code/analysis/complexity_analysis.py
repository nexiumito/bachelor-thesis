from analysis.SATTools import *
from notifications.discordNotification import send_discord_notification
from z3 import *

def complexity_evaluation(pairs: dict[int, list[tuple[int, int]]], dimacs_dir: str, simplifie: bool = False, bloat: bool = False) -> dict[int, list[float]]:

    time_per_bits = {}
    for size_bits, pair in pairs.items():
        times = []
        backjumps = []
        for x, y in pair:
            ts, _, _, _ = SAT_producer(x, y, simplifie=simplifie, bloat=bloat, repository=dimacs_dir)
            solver = Solver()
            solver.from_file(ts.dimacs)
            start_time = time.time()
            if solver.check() == sat:
                stats = solver.statistics()
                try:
                    backjump = stats.get_key_value('sat backjumps')
                except:
                    backjump = 0
                    
                finished_time = time.time() - start_time
                times.append(finished_time)
                backjumps.append(backjump)
            else: print("unsat")
        
        print(times, backjumps)    
        time_per_bits[size_bits] = (times, backjumps)
        if size_bits > 25:
            send_discord_notification(f"évaluation du temps terminée pour k = {size_bits} !")
        
    return time_per_bits